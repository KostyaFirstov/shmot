...
import ReviewsPageSkeleton from './ReviewsPage/ReviewsPageSkeleton'
import ContentLayout from '../layouts/ContentLayout'

const DropPage = () => {
	const [drop, setDrop] = React.useState<DropParams>()
	...
	React.useEffect(() => {
		const fetchDrop = async () => {
			setIsLoading(true)
			try {
				const { data } = await axios.get(`/api/drops/${params.title}`)
				setDrop(data)
				setIsLoading(false)
			} catch (error) {
				console.log(error)
			...
			{isLoading ? (
				...
				<>
					<div className='review-page__image'>
						<img src={`http://localhost:5000${drop?.img[0]}`} alt='' />
					</div>
					<div className='review-page__header'>
						...
						<div className='review-page__details'>
							<div className='review-page__date'>
								<TimeAgo date={drop ? drop.createdAt : ''} />
							</div>
							<div
								className='review-page__viewscount'
								title={`Просмотров ${drop?.viewsCount}`}
							>
								<span>{drop?.viewsCount}</span>
								<svg
									width='20'
									height='14'
									viewBox='0 0 20 14'
					...
					<div className='review-page__main'>
						<div className='review-page__date'>{drop?.date}</div>
